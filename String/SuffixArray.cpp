namespace atcoder {namespace internal {//remove long long
...
vector<int> sa_is(const vector<int>& s, int upper) {
    ...
    vector<int> lms_map(n + 1, -1);int m = 0;
    for (int i = 1; i < n; i++) {
        if (!ls[i - 1] && ls[i]) lms_map[i] = m++;
    }
    vector<int> lms;lms.reserve(m);
    for (int i = 1; i < n; i++)
        if (!ls[i - 1] && ls[i])lms.push_back(i);
    induce(lms);
    if (m) {
        vector<int> sorted_lms;sorted_lms.reserve(m);
        for (int v : sa)
...
vector<int> suffix_array(const string& s) {
    int n = int(s.size());vector<int> s2(n);
